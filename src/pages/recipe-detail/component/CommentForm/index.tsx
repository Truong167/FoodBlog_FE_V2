...
const CommentForm: React.FC<TCommentFormProps> = ({ recipeId, comments }) => {
  ...
  const items: MenuProps["items"] = [
    {
      key: "delete",
      label: <p onClick={() => setIsDeleteModalOpen(true)}>Xóa bình luận</p>,
    },
    {
      key: "edit",
      label: <p>Chỉnh sửa bình luận</p>,
    },
  ];
              ...
              <div className="flex" key={item.commentId}>
                <Meta
                  ...
                  avatar={
                    <Avatar
                      className="h-12 w-12"
                      src={`${item.User.avatar ? item.User.avatar : no_avatar}`}
                    />
                  }
                  title={
                ...
                {item.isMyComment && (
                  <Dropdown
                    menu={{
                      items,
                      onClick: (e) =>
                        handleActionComment(e, item.commentId, item.comment),
                    }}
                  >
